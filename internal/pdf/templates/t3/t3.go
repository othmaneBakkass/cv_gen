// Package t3 renders a CV using the "Modern Minimal" layout: a compact,
// indigo-accented design with title-case headings that carry no rule line
// (hierarchy comes from weight/color/spacing alone), an en-dash bullet
// marker, and tighter overall spacing than t2. It shares every drawing
// primitive with t1/t2 (internal/pdf/draw, internal/pdf/comp,
// internal/pdf/sections) — only the theme (internal/pdf/theme.T3) differs.
// See docs/pdf-rewrite-handoff.md for the layered architecture this and
// every other template is built on.
package t3
...
// Render builds the t3 layout for cv under opts and returns the finished
...
func Render(cv schema.CV, opts render.Options) ([]byte, error) {
	th := opts.EffectiveMargins(opts.ApplyTheme(theme.T3))
